...
export class AssignMacroPlanComponent implements OnInit, AfterViewInit {
  planName = '';
  planDescription = '';
  ...
  selectFood(f: any) {
    if (!this.mealForModal) return;
    this.mealForModal.protein += f.protein;
    this.mealForModal.carbs += f.carbs;
    this.mealForModal.fat += f.fat;
  ...
  recomputeAdjusted() {
    ...
    const grams =
      this.foodUnit === 'g' ? this.foodQty : this.foodQty * this.servingGram;
