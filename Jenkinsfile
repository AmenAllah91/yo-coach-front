        ...
        stage('Perform Service Update') {
            steps {
                sshCommand remote: [
                  ...
                  cd workspace-yocoach && sudo docker-compose pull ${SERVICE_NAME} && sudo docker-compose down ${SERVICE_NAME} && sudo docker-compose up -d --no-deps --force-recreate ${SERVICE_NAME}
                """
            }
        }
    }
